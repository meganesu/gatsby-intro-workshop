...
<Calloutbox>
  In this section, you will go through the process of creating and deploying
...
If you're running into trouble using the Gatsby CLI on your machine, you can follow along by forking this [CodeSandbox template](https://codesandbox.io/s/gatsby-workshop-starter-template-ol8c4).
...
To run your site locally, you'll first need to change directories in your terminal (with the `cd` command) so that you're in the folder that was created for your new site.
...
In this workshop, you'll use `gatsby develop`.
...
1. Select your GitHub username from the "Select an Organization" dropdown.
...
In order to use Gatsby Hosting, you first need to add the `gatsby-plugin-gatsby-cloud` plugin to your site. (Don't worry about what this means for now. You'll learn more about plugins in a bit.)

1. Run the following command from your terminal to install `gatsby-plugin-gatsby-cloud`:
    ```shell
    npm install gatsby-plugin-gatsby-cloud
    ```
1. Open up the `gatsby-config.js` file at the root of your project. Update the `plugins` array to look like this:
    ```javascript
    plugins: [
      `gatsby-plugin-gatsby-cloud`,
    ],
    ```
1. Commit your changes and push them to your remote repository on GitHub:
    ```shell
    git add .
    git commit -m "Add plugin for hosting"
    git push
    ```

Then, you can set up Gatsby Hosting in your browser:

1. Go to the Gatsby Cloud page for your site, then click into the "Site Settings" tab.
...
Now you can access your site using the `.gatsbyjs.io` URL listed under "Connected domains"!

## Exercise #1: Create and deploy your first Gatsby site

Create a new Gatsby site, then build and host it with Gatsby Cloud.

When you're done, you should have a `.gatsbyjs.io` URL to your site that you can share in the chat.
...
https://github.com/meganesu/gatsby-intro-workshop-example-site-with-v3/commit/2eb471ff6983727bd8a246a412947c68ca9e90c1).
...
You can help us improve this workshop by using this [feedback form](https://docs.google.com/forms/d/e/1FAIpQLScQSMjKfAaukPiiwvPTpIG3nKEF5pjY0Y_nZX6TGj0L9Tx31w/viewform?usp=pp_url&entry.2075407804=Part+1) to tell us what you liked or didn't like about this part of the workshop.
